...
public class TrajectoryPlanner {
    ...
    private static double _launchAngle[] = {
            0.13,
            0.215,
            0.296,
            0.381,
            ...
            1.106,
            1.197 };
    private static double _changeAngle[] = {
            0.052,
            0.057,
            0.063,
            0.066,
            ...
            0.029,
            0.025 };
    private static double _launchVelocity[] = {
            2.9,
            2.88,
            2.866,
            2.838,
     ...
     *
     * @param sling - bounding rectangle of the slingshot releasePoint - point the
     * mouse click is released from x - x-coordinate (on screen) of the requested
     * point
     * 
    ...
    /*
     * Estimate launch points given a desired target point using maximum velocity If
     * there are two launch point for the target, they are both returned in the list
     * {lower point, higher point) Note - angles greater than 75 are not considered
     *
    ...
    public ArrayList<Point> estimateLaunchPoint(Rectangle slingshot, Point targetPoint) {
        System.out.println("sling: " + slingshot);
    ...
    /*
     * Choose a trajectory by specifying the sling location and release point Derive
     * all related parameters (angle, velocity, equation of the parabola, etc)
     *
     * @param sling - bounding rectangle of the slingshot releasePoint - point where
     * the mouse click was released from
     */
     ...
     * Plot a trajectory
     *
     * @param canvas - the canvas to draw onto slingshot - bounding rectangle of the
     * slingshot releasePoint - point where the mouse click was released from
     * 
     ...
     * maximum velocity
     *
     * @param sling - bounding rectangle of the slingshot theta - launch angle in
     * radians (anticlockwise from positive direction of the x-axis)
     * 
     ...
     *
     * @param sling - bounding rectangle of the slingshot theta - launch angle in
     * radians (anticlockwise from positive direction of the x-axis) v - exit
    ...
    // take the initial angle of the desired trajectory and return the launch angle
    // required
    ...
    public int getTapTimeYellowBird(Rectangle sling, Point release, Point target,
    		int impactAngle) {
    	setTrajectory(sling, release);
    	// parameters of approximating linear movement after acceleration
    	double impact_gradient = Math.tan(impactAngle * Math.PI / 180.0d);
    	System.out.println("impact gradient: " + impact_gradient);
    	System.out.println("_a: " + _a  + " , _b: " + _b);
    	double tap_x  = (impact_gradient - _b) / 2 / _a;
    	System.out.println("scene scale: " + getSceneScale(sling));
    	tap_x = tap_x * getSceneScale(sling);
    	System.out.println("tap_x: " + tap_x);
    	
    	// i think the equation below is not what we are looking for...
    	// double tap_x  = (_b - n) / (2 * _a - impact_gradient);
    	// sometimes, the equation yields a point behind the target because we
    	// do not apply some offset to move the virtual target to the left. In 
    	// this case make sure the bird is activated right before its impact.
    	if (tap_x + sling.x >= target.x) {
    		System.out.println("actual tap point is too late!");
    		// old approach: small x offset
    		tap_x = target.x - sling.x - 30;
    		// new approach trigger by height over target
    		// tap_x = Math.sqrt((target.y - 30) / Math.abs(_a) / getSceneScale(sling) + _b * _b / 4 / _a / _a) - _b / 2 / _a;
    		// tap_x = tap_x * getSceneScale(sling);
    		System.out.println("tap_x after correction: " + tap_x);
    	}
    	Point tap_point = new Point((int) (tap_x + sling.x), 
    			                     target.y);
    	System.out.println("yellow tap point: " + tap_point);
    	return(getTimeByDistance(sling, release, tap_point));
    }
}
