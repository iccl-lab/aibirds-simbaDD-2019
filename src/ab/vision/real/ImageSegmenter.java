...
public class ImageSegmenter {
     ...
     * 
     * @return boolean map in the form isEdge[y][x], where isEdge[y][x] means point
     * (x, y) is an edge
     */
    private boolean[][] findEdges() {
        ...
        for (int y = _height - 3; y > 1; y--)
            for (int x = 2; x < _width - 2; x++) {
                if (isEdge[y][x][0] || isEdge[y][x][1] || isEdge[y][x][2] || isEdge[y][x][3])
                    ret[y][x] = true;
            }
     ...
     * 
     * @param canvas to draw onto if the corners should be indicated
     */
     ...
     * Initialise the drawing color for each types - Set minimum and maximum size of
     * each type of object
     */
    ...
    /*
     * Assign the 15-bit color to the most likely type using nearest neighbour in
     * the training data
     * 
