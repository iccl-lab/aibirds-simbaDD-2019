...
public class ConnectedComponent {
     ...
     * 
     * @param map - class map of the screenshot x,y - starting coordinate of the
     * connected component ignore - points on the map to ignore isEightConnect -
     * whether the component is eight-connected or four-connected
     */
    public ConnectedComponent(final int map[][], int x, int y, boolean ignore[][], boolean isEightConnect) {
        ...
        while (!q.isEmpty()) {
            ...
            if (map[p.y][p.x] == _type || (_type == ImageSegmenter.ICE && map[p.y][p.x] == ImageSegmenter.BLUE_BIRD)
    ...
    /*
     * Trace the contour using Moore-Neighbour tracing and partition contour points
     * into line segments
     * 
     * @return list of line segments which form a cycle around outer border of the
     * connected component
     */
    ...
    /*
     * Determine if the connected component is a Rectangle, Circle or in fact
     * multiple objects stacked together.
     *
     * @param list of corners the component contains (points where contour
     * orientation changes)
     * 
    ...
    public Body getBody() {
        ...
        if ((_type > ImageSegmenter.SLING && _type <= ImageSegmenter.PIG) || _type == ImageSegmenter.TRAJECTORY)
     ...
     * 
     * @param canvas drawEdge - if border should be drawn drawCorner - if the corner
    ...
    private static void initialise(int map[][]) {
        if (_connectedPoint != null && _connectedPoint.length == map.length
    ...
    private final static int xClock[][] = {
            {
                    0,
                    -1,
                    -1 },
                    ...
                    0 },
            {
                    ...
                    1,
                    0 } };
    private final static int yClock[][] = {
            {
                    1,
                    0,
                    0 },
     ...
     * 
     * @param p - the current contour point prev - point just examined
     * 
     * @return point connected to p which is in the anticlockwise direction from p
     * to prev
     */
