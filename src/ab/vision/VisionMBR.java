...
public class VisionMBR {
        ...
        for (int i = 0; i < _nHeight; i++) {
            for (int j = 0; j < _nWidth; j++) {
                ...
                while (true) {
                    ...
                    if (p.y < _nHeight - 1)
                        if ((_scene[p.y + 1][p.x] == 345 || _scene[p.y + 1][p.x] == 418 || _scene[p.y + 1][p.x] == 273
                                || _scene[p.y + 1][p.x] == 281 || _scene[p.y + 1][p.x] == 209
                    ...
                    // check right pixel
                    if (p.x < _nWidth - 1)
                        if ((_scene[p.y][p.x + 1] == 345 || _scene[p.y][p.x + 1] == 418 || _scene[p.y][p.x + 1] == 346
                                || _scene[p.y][p.x + 1] == 354 || _scene[p.y][p.x + 1] == 273
                                || _scene[p.y][p.x + 1] == 281 || _scene[p.y][p.x + 1] == 209
                    ...
                    // check upper pixel
                    if (p.y > 0)
                        if ((_scene[p.y - 1][p.x] == 345 || _scene[p.y - 1][p.x] == 418 || _scene[p.y - 1][p.x] == 346
                                || _scene[p.y - 1][p.x] == 354 || _scene[p.y - 1][p.x] == 273
                                || _scene[p.y - 1][p.x] == 281 || _scene[p.y - 1][p.x] == 209
                                || _scene[p.y - 1][p.x] == 282 || _scene[p.y - 1][p.x] == 351)
                                && !ignorePixel[p.y - 1][p.x]) {
                            l.add(new Point(p.x, p.y - 1));
                    ...
                    if (_scene[obj.y][obj.x] == 511 || _scene[obj.y][obj.x] == 447) {
                        for (int m = obj.y; m < obj.y + obj.height; m++) {
                            if (_scene[m][obj.x] == 345 || _scene[m][obj.x] == 418 || _scene[m][obj.x] == 346
                                    ...
                                    || _scene[m][obj.x] == 209 || _scene[m][obj.x] == 282 || _scene[m][obj.x] == 351) {
                ...
                if ((hist[345] > Math.max(32, 0.1 * obj.width * obj.height)) && (hist[64] != 0)) {
                    obj.add(
                            new Rectangle(obj.x - obj.width / 10, obj.y - obj.height / 3, obj.width / 10 * 12,
                                    obj.height / 3 * 4));
                    return obj;
                }
            }
    ...
    public List<Rectangle> findPigsMBR() {
        ...
        for (int n = 0; n < _nSegments; n++) {
            ...
            Rectangle bounds = VisionUtils
                    .dialateRectangle(_boxes[n], _boxes[n].width / 2 + 1, _boxes[n].height / 2 + 1);
            ...
            for (int m = n + 1; m < _nSegments; m++) {
                if (_colours[m] != 376)
                    continue;
                final Rectangle bounds2 = VisionUtils
                        .dialateRectangle(_boxes[m], _boxes[m].width / 2 + 1, _boxes[m].height / 2 + 1);
    ...
    public List<ABObject> findBirds() {
        ...
        List<ABObject> objects = new LinkedList<ABObject>();
        objects.addAll(constructABObjects(rbirds, ABType.RED_BIRD));
        objects.addAll(constructABObjects(ybirds, ABType.YELLOW_BIRD));
        objects.addAll(constructABObjects(blbirds, ABType.BLUE_BIRD));
        objects.addAll(constructABObjects(blackbirds, ABType.BLACK_BIRD));
        objects.addAll(constructABObjects(wbirds, ABType.WHITE_BIRD));
        return objects;
    }
    ...
    public List<Rectangle> findStonesMBR() {
        ...
        for (int i = 0; i < _nHeight; i++) {
            for (int j = 0; j < _nWidth; j++) {
                ...
                if (obj.width * obj.height > _regionThreshold && !(new Rectangle(0, 0, 190, 55).contains(obj)))
                    objects.add(obj);
            }
        }
        return objects;
        ...
        for (int i = 0; i < _nHeight; i++) {
            for (int j = 0; j < _nWidth; j++) {
                ...
                while (true) {
                    ...
                    // belong to ice
                    if (p.y < _nHeight - 1)
                        if ((_scene[p.y + 1][p.x] == 311 || _scene[p.y + 1][p.x] == 247 || _scene[p.y + 1][p.x] == 183)
                                && !ignorePixel[p.y + 1][p.x]) {
                            l.add(new Point(p.x, p.y + 1));
                            obj.add(p.x, p.y + 1);
                        }
                    if (p.x < _nWidth - 1)
                        if ((_scene[p.y][p.x + 1] == 311 || _scene[p.y][p.x + 1] == 247 || _scene[p.y][p.x + 1] == 183)
                                && !ignorePixel[p.y][p.x + 1]) {
                            l.add(new Point(p.x + 1, p.y));
                            obj.add(p.x + 1, p.y);
                        }
        ...
        for (int i = 0; i < _nHeight; i++) {
            for (int j = 0; j < _nWidth; j++) {
                ...
                while (true) {
                    ...
                    // belong to wood
                    if (p.y < _nHeight - 1)
                        if ((_scene[p.y + 1][p.x] == 481 || _scene[p.y + 1][p.x] == 408 || _scene[p.y + 1][p.x] == 417)
                                && !ignorePixel[p.y + 1][p.x]) {
                            l.add(new Point(p.x, p.y + 1));
                            obj.add(p.x, p.y + 1);
                    ...
                    if (p.x < _nWidth - 1)
                        if ((_scene[p.y][p.x + 1] == 481 || _scene[p.y][p.x + 1] == 408 || _scene[p.y][p.x + 1] == 417)
                                && !ignorePixel[p.y][p.x + 1]) {
                            l.add(new Point(p.x + 1, p.y));
                            obj.add(p.x + 1, p.y);
            ...
            for (int j = 0; j < _nWidth; j++) {
                ...
                while (true) {
                    ...
                    if (p.y < _nHeight - 1 && p.x < _nWidth - 1 && p.y > 0 && p.x > 0) {
                        if ((_scene[p.y + 1][p.x] == 365 || _scene[p.y + 1][p.x] == 366 || _scene[p.y + 1][p.x] == 438)
                                && !ignorePixel[p.y + 1][p.x]) {
                            l.add(new Point(p.x, p.y + 1));
                            obj.add(p.x, p.y + 1);
                        }

                        if ((_scene[p.y][p.x + 1] == 365 || _scene[p.y][p.x + 1] == 366 || _scene[p.y][p.x + 1] == 438)
                                && !ignorePixel[p.y][p.x + 1]) {
                            l.add(new Point(p.x + 1, p.y));
                            obj.add(p.x + 1, p.y);
                        }
