        ...
        if (!shots.isEmpty()) {
            ...
            if (shots.size() < 3) {
                ...
                if (shots.size() == 2) {
                    ...
                    int wait_time = (_shot.getT_shot() - start_time) == 0 ? start_time
                            : (_shot.getT_shot() - start_time);
                    ...
                    try {
                       ...
                       * long _time = System.nanoTime(); System.out.println(" waiting time:" + (time -
                       * _time));
                       */
                    proxy.send(new ProxyDragMessage(_shot.getX(), _shot.getY(), _shot.getDx(), _shot.getDy()));
                } else {
                    proxy.send(new ProxyDragMessage(shot.getX(), shot.getY(), shot.getDx(), shot.getDy()));
                    // System.out.println(shot.getX() + " " + shot.getY() + " " + shot.getDx() + " "
                    // + shot.getDy());
                }
                ...
                for (Shot _shot : shots) {
                    long wait_time = (_shot.getT_shot() - start_time - gap) <= 0 ? 0
                            : (_shot.getT_shot() - start_time - gap);
